...
dynamic_tensor moe_weight_provider::get(const std::string& tensor_name, int expert_id) {
  ...
  if (tensor_info.type == ggml_type::Q4_K || tensor_info.type == ggml_type::MXFP4 || tensor_info.type == ggml_type::F16) {
    ...
    throw std::runtime_error("Unsupported tensor type: " + std::to_string(static_cast<int>(tensor_info.type)));
  }
  ...
  if (tensor_info.type == ggml_type::Q4_K) {
    ...
    if (is_1d_output) {
      ...
      result = dynamic_tensor(dtype::float32, {static_cast<int64_t>(dim0), static_cast<int64_t>(dim1)});
    }
...
size_t moe_weight_provider::get_max_cache_size() const {
...
void moe_weight_provider::evict_lru_until_space(size_t required_bytes) {
  while (current_cache_size_bytes_ + required_bytes > max_cache_size_bytes_ && !cache_lru_list_.empty()) {
