...
void gpt_oss_engine::build_transformer_graph() {
  ...
  for (int layer = 0; layer < pimpl_->num_layers; ++layer) {
    ...
    for (int expert_id = 0; expert_id < pimpl_->num_experts; ++expert_id) {
      ...
      expert_sync->set_initial_ids({layer_prefix + ".ffn_norm_out", layer_prefix + ".ffn_up_exps.weight", layer_prefix + ".ffn_gate_exps.weight", layer_prefix + ".ffn_down_exps.weight", layer_prefix + ".ffn_up_exps.bias", layer_prefix + ".ffn_gate_exps.bias", layer_prefix + ".ffn_down_exps.bias"});
      pimpl_->graph->add_node(expert_sync);

      expert->set_input(expert_sync->get_output(), "default");
      expert->set_input_names({layer_prefix + ".ffn_norm_out", layer_prefix + ".ffn_up_exps.weight", layer_prefix + ".ffn_gate_exps.weight", layer_prefix + ".ffn_down_exps.weight", layer_prefix + ".ffn_up_exps.bias", layer_prefix + ".ffn_gate_exps.bias", layer_prefix + ".ffn_down_exps.bias"});
