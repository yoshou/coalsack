...
namespace coalsack
{
    ...
    public:
        session(tcp::socket socket, uint32_t session_id, const std::unordered_map<uint32_t, rpc_func> &handlers, const rpc_disconnect_func disconnect_handler)
            : socket_(std::move(socket)), receive_buff_(), session_id(session_id), handlers_(handlers), disconnect_handler(disconnect_handler)
        {
        }
    ...
    public:
        ...
        void register_handler(uint32_t id, rpc_func func)
        {
        ...
        void on_discconect(rpc_disconnect_func handler)
        {
        ...
        void do_accept()
        {
            acceptor_.async_accept(socket_,
                                   [this](boost::system::error_code ec)
                                   {
                                       if (!ec)
                                       {
                                           std::make_shared<session>(std::move(socket_), next_session_id, handlers, disconnect_handler)->start();
                                           next_session_id++;
                                       }
