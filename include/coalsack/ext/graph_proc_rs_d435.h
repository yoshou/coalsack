...
const std::vector<stream_index_pair> IMAGE_STREAMS = {DEPTH, INFRA0,  INFRA1,   INFRA2,
                                                      COLOR, FISHEYE, FISHEYE1, FISHEYE2};

struct stream_profile_request {
  stream_profile_request(rs2_format_type format = rs2_format_type::ANY,
                         rs2_stream_type stream = rs2_stream_type::ANY, int index = 0,
                         uint32_t width = 0, uint32_t height = 0, uint32_t fps = 0)
...
class rs_d435_node : public graph_node {
 ...
 public:
  rs_d435_node() : graph_node() {}

  virtual std::string get_proc_name() const override { return "rs_d435"; }

  void set_serial_number(std::string serial_number) { this->serial_number = serial_number; }

  std::string get_serial_number() const { return serial_number; }

  graph_edge_ptr get_output(stream_index_pair stream, int width, int height,
                            rs2_format_type format = rs2_format_type::ANY, int fps = 0) {
  ...
  graph_edge_ptr add_output(stream_index_pair stream, int width = 0, int height = 0,
                            rs2_format_type format = rs2_format_type::ANY, int fps = 0) {
    const auto [stream_type, stream_index] = stream;
    stream_profile_request profile(format, stream_type, stream_index, width, height, fps);
    std::string output_name = "stream" + std::to_string(get_stream_profile_request_id(profile));
  ...
  virtual void run() override {
    ...
    for (auto profile : request_profiles) {
      ...
      config.enable_stream(stream, profile.index, profile.width, profile.height, format,
                           profile.fps);
    }
...
#else
  virtual void run() override { throw std::runtime_error("Not implemented"); }
  virtual void stop() override { throw std::runtime_error("Not implemented"); }
#endif
 ...
 private:
  ...
  void video_frame_callback(rs2::video_frame frame) {
    ...
    msg->set_profile(std::make_shared<stream_profile>(
        convert_stream_type(profile.stream_type()), profile.stream_index(),
        convert_stream_format(profile.format()), profile.fps(), profile.unique_id()));
    ...
    if (auto video_profile = profile.as<rs2::video_stream_profile>()) {
      ...
      auto output =
          get_output(sip, video_profile.width(), video_profile.height(), format, profile.fps());
      output->send(msg);
    }
  }
