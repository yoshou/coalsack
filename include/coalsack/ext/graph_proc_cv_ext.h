...
#include "coalsack/ext/blob_detector.h"
...
class fast_blob_detector_node : public graph_node {
 ...
 public:
  fast_blob_detector_node() : graph_node(), output(std::make_shared<graph_edge>(this)), params() {
    set_output(output);
  }
...
class charuco_detector_node : public graph_node {
 ...
 public:
  ...
  virtual void initialize() override {
    ...
    const auto board =
        cv::aruco::CharucoBoard(board_size, square_length, marker_length, dictionary);
    detector = cv::makePtr<cv::aruco::CharucoDetector>(board, charuco_params, detector_params);
#else
    const auto dictionary = cv::aruco::getPredefinedDictionary(dict_type);
    board = cv::aruco::CharucoBoard::create(board_size.width, board_size.height, square_length,
    ...
    if (auto image_msg = std::dynamic_pointer_cast<frame_message<image>>(message)) {
      ...
      try {
        ...
        if (marker_ids.size() > 0) {
          cv::aruco::interpolateCornersCharuco(marker_corners, marker_ids, src_mat, board,
