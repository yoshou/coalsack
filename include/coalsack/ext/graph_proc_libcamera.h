...
#include "coalsack/ext/libcamera_capture.h"
...
class libcamera_capture_node : public graph_node {
 ...
 public:
    ...
    th.reset(new std::thread([this]() {
      while (running.load()) {
        libcamera_capture::buffer buffer;
        {
          ...
          cv.wait(lock, [&] { return !frames.empty() || !running; });
          if (!running) {
            break;
          }
          if (!frames.empty()) {
...

        {
          ...
          msg->set_profile(
              std::make_shared<stream_profile>(stream, 0, stream_fmt, camera->get_framerate(), 0));
  ...
  void set_format(image_format value) { format = value; }
