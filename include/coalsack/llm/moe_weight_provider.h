...
class moe_weight_provider {
 ...
 public:
  explicit moe_weight_provider(std::shared_ptr<gguf_multi_loader> loader,
 ...
 private:
  ...
  std::vector<uint8_t> load_expert_slice_from_file(uint32_t shard_idx, uint64_t offset,
                                                   size_t num_bytes);
  void load_expert_slice_from_file(uint32_t shard_idx, uint64_t offset, uint8_t* buffer,
                                   size_t num_bytes);
};
