...
#include "coalsack/data_stream/data_stream_common.h"
 ...
 public:
  ...
  void send(source_identifier id, double timestamp, const uint8_t *data, size_t length) {
    ...
    for (size_t pos = 0; pos < length; pos += max_payload_size) {
      ...
      size_t send_size = socket_.send_to(boost::asio::buffer(buffer_, payload_size + header_size),
                                         remote_endpoint_);
...
class data_stream_tcp_transmitter {
 ...
 public:
  ...
  void open(std::string address, uint16_t port) {
    ...
    if (error) {
      throw std::runtime_error("Failed connecting to " + address + ":" + std::to_string(port) +
                               " (" + error.message() + ")");
    }
  }
  void close() {
  ...
  void send(source_identifier id, double timestamp, const uint8_t *data, size_t length) {
    ...
    for (size_t pos = 0; pos < length; pos += max_payload_size) {
      ...
      boost::system::error_code error;
      asio::write(socket_, asio::buffer(boost::asio::buffer(buffer_, payload_size + header_size)),
                  error);
