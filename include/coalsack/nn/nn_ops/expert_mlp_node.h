...
#include "coalsack/tensor/dynamic_mx_tensor_message.h"
 ...
 private:
  ...
  __attribute__((target("f16c,avx2,fma"))) void compute_token_mxfp4(
    ...
    if (expert_ffn_dim % TILE != 0) {
      throw std::runtime_error("compute_token_mxfp4: expert_ffn_dim must be even, got " +
                               std::to_string(expert_ffn_dim));
    }
    if (hidden_dim % TILE != 0) {
      throw std::runtime_error("compute_token_mxfp4: hidden_dim must be even, got " +
                               std::to_string(hidden_dim));
    }
