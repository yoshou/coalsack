...
static void create_gaussian_kernel(float *kernel, int size, float sigma) {
  ...
  for (int x = -r, i = 0; x <= r; x++, i++) {
    const auto value = static_cast<float>(std::exp(-(x * x / (2 * sigma * sigma))) /
                                          std::sqrt(2 * pi * sigma * sigma));
    kernel[i] = value;
    total += value;
  }
  ...
  if (width - radius_x >= num_vector_lanes) {
    for (; x <= width - radius_x - num_vector_lanes; x += num_vector_lanes) {
      ...
      if (kernel_width == 3 && symmetric) {
        ...
        const float32x4_t v_acc0 = vmlaq_f32(vmulq_f32(vcvtq_f32_u32(v_src02_0), v_coeff0),
                                             vcvtq_f32_u32(v_src1_0), v_coeff1);
        const float32x4_t v_acc1 = vmlaq_f32(vmulq_f32(vcvtq_f32_u32(v_src02_1), v_coeff0),
                                             vcvtq_f32_u32(v_src1_1), v_coeff1);
        const float32x4_t v_acc2 = vmlaq_f32(vmulq_f32(vcvtq_f32_u32(v_src02_2), v_coeff0),
                                             vcvtq_f32_u32(v_src1_2), v_coeff1);
        const float32x4_t v_acc3 = vmlaq_f32(vmulq_f32(vcvtq_f32_u32(v_src02_3), v_coeff0),
                                             vcvtq_f32_u32(v_src1_3), v_coeff1);
  ...
  if (width >= num_vector_lanes) {
    for (; x <= width - num_vector_lanes; x += num_vector_lanes) {
      if (kernel_height == 3 && symmetric) {
        ...
        const float32x4_t v_acc0 =
            vmlaq_f32(vmulq_f32(vaddq_f32(v_src0_0, v_src2_0), v_coeff0), v_src1_0, v_coeff1);
        const float32x4_t v_acc1 =
            vmlaq_f32(vmulq_f32(vaddq_f32(v_src0_1, v_src2_1), v_coeff0), v_src1_1, v_coeff1);
        const float32x4_t v_acc2 =
            vmlaq_f32(vmulq_f32(vaddq_f32(v_src0_2, v_src2_2), v_coeff0), v_src1_2, v_coeff1);
        const float32x4_t v_acc3 =
            vmlaq_f32(vmulq_f32(vaddq_f32(v_src0_3, v_src2_3), v_coeff0), v_src1_3, v_coeff1);
...
static void gaussian_blur(cv::Mat src_mat, cv::Mat dst_mat, int kernel_width, int kernel_height,
  ...
  if (dst_mat.channels() == 1) {
    ...
    for (std::size_t y = 0; y < static_cast<std::size_t>(kernel_height) - 1; y++) {
      filter_row(&src_mat.data[y * stride], width, kernel_x.get(), kernel_width,
                 &row_buffer[(y % kernel_height) * width]);
    }
    ...
    for (std::size_t y = radius_y; y < height - radius_y; y++) {
      filter_row(&src_mat.data[(y + radius_y) * stride], width, kernel_x.get(), kernel_width,
                 &row_buffer[((y + radius_y) % kernel_height) * dst_mat.cols]);
      ...
      filter_col(row_ptrs.get(), width, kernel_y.get(), static_cast<std::size_t>(kernel_height),
                 dst_row);
    }
