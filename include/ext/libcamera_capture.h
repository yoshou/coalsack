...
class libcamera_capture {
 public:
  ...
  struct stream_configuration {
    stream_configuration(stream_format format, uint32_t width = 0, uint32_t height = 0,
                         uint32_t fps = 0)
 ...
 private:
      ...
      const auto timestamp =
          (std::chrono::duration_cast<std::chrono::nanoseconds>(
           ...
           (metadata.timestamp - std::chrono::duration_cast<std::chrono::nanoseconds>(
                                     steady_clock_start.time_since_epoch())
                                     .count())) /
          1000000.0;
  ...
  void set_default_control() {
    ...
    if (!controls.get(libcamera::controls::ScalerCrop) && roi_width != 0 && roi_height != 0) {
      libcamera::Rectangle sensor_area =
 ...
 public:
  libcamera_capture()
      : running(false),
        stream(nullptr),
        controls(libcamera::controls::controls),
        duration(1000.0 / 30) {}
  ...
  void open(std::size_t camera_idx) {
    ...
    if (camera->acquire()) throw std::runtime_error("Failed to acquire camera " + cam_id);
  ...
  void set_interval(double time_ms) { this->duration = time_ms; }
  void set_exposure_time(int32_t time_us) {
    controls.set(libcamera::controls::ExposureTime, time_us);
  }
  void set_gain(float value) { controls.set(libcamera::controls::AnalogueGain, value); }
  void set_color_gain(float value_r, float value_b) {
    controls.set(libcamera::controls::ColourGains,
                 libcamera::Span<const float, 2>({value_r, value_b}));
  }
  void set_brightness(float value) { controls.set(libcamera::controls::Brightness, value); }
  void set_contrast(float value) { controls.set(libcamera::controls::Contrast, value); }
  void set_sharpness(float value) { controls.set(libcamera::controls::Sharpness, value); }
    ...
    th.reset(new std::thread([this]() {
      while (running.load()) {
...

        {
          ...
          cv.wait(lock, [&] { return !res.empty() || !running; });
  ...
  void stop() {
    ...
    }
    {
      ...
      if (camera->stop()) throw std::runtime_error("failed to stop camera");
    }
