    ...
    public:
        ...
        virtual void process(std::string input_name, graph_message_ptr message) override
        {
            if (auto image_msg = std::dynamic_pointer_cast<frame_message<image>>(message))
            {
                ...
                const auto compressed_size = LZ4_compress_default((const char *)src_buf, (char *)dst_buf.data() + header_size, src_size, max_dst_size);
    ...
    public:
        ...
        virtual void process(std::string input_name, graph_message_ptr message) override
        {
            if (auto image_msg = std::dynamic_pointer_cast<frame_message<blob>>(message))
            {
                ...
                const int decompressed_size = LZ4_decompress_safe((const char *)src_buf + header_size, (char *)img.get_data(), src_size - header_size, dst_size);
