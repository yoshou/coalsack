...
class pre_inference_node : public graph_node {
 ...
 public:
  pre_inference_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }
    ...
    if (auto obj_msg = std::dynamic_pointer_cast<object_message>(message)) {
      ...
      for (const auto &[name, field] : obj_msg->get_fields()) {
        if (auto frame_msg = std::dynamic_pointer_cast<frame_message<tensor<float, 4>>>(field)) {
          ...
          cameras_rotation.push_back(make_tensor<float, 3>(
              {static_cast<float>(camera.rotation[0][0]), static_cast<float>(camera.rotation[0][1]),
               static_cast<float>(camera.rotation[0][2]), static_cast<float>(camera.rotation[1][0]),
               static_cast<float>(camera.rotation[1][1]), static_cast<float>(camera.rotation[1][2]),
               static_cast<float>(camera.rotation[2][0]), static_cast<float>(camera.rotation[2][1]),
               static_cast<float>(camera.rotation[2][2])},
              {1, 3, 3}));
          cameras_translation.push_back(make_tensor<float, 3>(
              {static_cast<float>(camera.translation[0]), static_cast<float>(camera.translation[1]),
               static_cast<float>(camera.translation[2])},
              {1, 3, 1}));
          cameras_standard_translation.push_back(
              make_tensor<float, 3>({static_cast<float>(standard_translation[0]),
                                     static_cast<float>(standard_translation[1]),
                                     static_cast<float>(standard_translation[2])},
                                    {1, 3, 1}));
          cameras_k.push_back(make_tensor<float, 3>(
              {static_cast<float>(camera.k[0]), static_cast<float>(camera.k[1]),
               static_cast<float>(camera.k[2])},
              {1, 3, 1}));
          cameras_p.push_back(make_tensor<float, 3>(
              {static_cast<float>(camera.p[0]), static_cast<float>(camera.p[1])}, {1, 2, 1}));

          images_center.push_back(make_tensor<float, 2>(
              {static_cast<float>(roi.center[0]), static_cast<float>(roi.center[1])}, {1, 2}));
          images_scale.push_back(make_tensor<float, 2>(
              {static_cast<float>(roi.scale[0]), static_cast<float>(roi.scale[1])}, {1, 2}));
          images_rotation.push_back(
              make_tensor<float, 2>({static_cast<float>(roi.rotation)}, {1, 1}));
          ...
          images_transform.push_back(make_tensor<float, 3>(
              {image_trans.at<float>(0, 0), image_trans.at<float>(0, 1),
               image_trans.at<float>(0, 2), image_trans.at<float>(1, 0),
               image_trans.at<float>(1, 1), image_trans.at<float>(1, 2), 0, 0, 1},
              {1, 3, 3}));
      ...
      const auto camera_standard_translation =
          tensor<float, 3>::stack(cameras_standard_translation);
...
class post_inference_node : public graph_node {
 ...
 public:
  post_inference_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }
