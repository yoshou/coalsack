 ...
 public:
  ...
  virtual void tick() override {
    ...
    const auto input_img_tensor_f = input_img_tensor.cast<float>().transform(
        [this](const float value, const size_t w, const size_t h, const size_t c, const size_t n) {
          return value / 255.0f;
        });
...
class parameter_loader_node : public graph_node {
 ...
 public:
  parameter_loader_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }
    ...
    if (auto frame_msg = std::dynamic_pointer_cast<frame_message_base>(message)) {
      ...
      for (const auto &item : params.items()) {
        ...
        if (shape.size() == 2) {
          ...
          tensor<float, num_dims> output_tensor(
...
class prepare_body_mesh_parameter_node : public graph_node {
 ...
 public:
  prepare_body_mesh_parameter_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }
  ...
  virtual void process(std::string input_name, graph_message_ptr message) override {
    if (auto obj_msg = std::dynamic_pointer_cast<object_message>(message)) {
      ...
      const auto hand_rotmat_mean =
          rot6d_to_rotmat(pred_rhand.view<3>({6, pred_rhand.shape[0] / 6, pred_rhand.shape[1]}));
      const auto left_hand_pose = flip_rotmat(hand_rotmat_mean);
      const auto right_hand_pose = hand_rotmat_mean;

      const auto pred_face_rotmat =
          rot6d_to_rotmat(pred_face.view<3>({6, pred_face.shape[0] / 6, pred_face.shape[1]}));
      const auto jaw_pose = pred_face_rotmat
                                .view<4>({pred_face_rotmat.shape[0], pred_face_rotmat.shape[1], 1,
                                          pred_face_rotmat.shape[3]},
                                         {0, 0, 0, 0})
                                .contiguous();
      const auto leye_pose = pred_face_rotmat
                                 .view<4>({pred_face_rotmat.shape[0], pred_face_rotmat.shape[1], 1,
                                           pred_face_rotmat.shape[3]},
                                          {0, 0, 1, 0})
                                 .contiguous();
      const auto reye_pose = pred_face_rotmat
                                 .view<4>({pred_face_rotmat.shape[0], pred_face_rotmat.shape[1], 1,
                                           pred_face_rotmat.shape[3]},
                                          {0, 0, 2, 0})
                                 .contiguous();
      const auto expression = pred_exp;
      const auto pred_rotmat_body =
          rot6d_to_rotmat(pred_pose.view<3>({6, pred_pose.shape[0] / 6, pred_pose.shape[1]}));
      const auto body_pose = pred_rotmat_body
                                 .view<4>({pred_rotmat_body.shape[0], pred_rotmat_body.shape[1], 21,
                                           pred_rotmat_body.shape[3]},
                                          {0, 0, 1, 0})
                                 .contiguous();
      const auto global_orient = pred_rotmat_body
                                     .view<4>({pred_rotmat_body.shape[0], pred_rotmat_body.shape[1],
                                               1, pred_rotmat_body.shape[3]},
                                              {0, 0, 0, 0})
                                     .contiguous();
...
class prepare_face_mesh_parameter_node : public graph_node {
 ...
 public:
  prepare_face_mesh_parameter_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }
  ...
  virtual void process(std::string input_name, graph_message_ptr message) override {
    if (auto obj_msg = std::dynamic_pointer_cast<object_message>(message)) {
      ...
      const auto pred_pose_rotmat =
          rot6d_to_rotmat(pred_pose.view<3>({6, pred_pose.shape[0] / 6, pred_pose.shape[1]}));
      const auto jaw_pose = pred_pose_rotmat
                                .view<4>({pred_pose_rotmat.shape[0], pred_pose_rotmat.shape[1], 1,
                                          pred_pose_rotmat.shape[3]},
                                         {0, 0, 0, 0})
                                .contiguous();
      const auto leye_pose = pred_pose_rotmat
                                 .view<4>({pred_pose_rotmat.shape[0], pred_pose_rotmat.shape[1], 1,
                                           pred_pose_rotmat.shape[3]},
                                          {0, 0, 1, 0})
                                 .contiguous();
      const auto reye_pose = pred_pose_rotmat
                                 .view<4>({pred_pose_rotmat.shape[0], pred_pose_rotmat.shape[1], 1,
                                           pred_pose_rotmat.shape[3]},
                                          {0, 0, 2, 0})
                                 .contiguous();
      const auto pred_orient_rotmat =
          rot6d_to_rotmat(pred_orient.view<3>({6, pred_orient.shape[0] / 6, pred_orient.shape[1]}));
      const auto global_orient =
          pred_orient_rotmat
              .view<4>({pred_orient_rotmat.shape[0], pred_orient_rotmat.shape[1], 1,
                        pred_orient_rotmat.shape[3]},
                       {0, 0, 0, 0})
              .contiguous();
...
class prepare_hand_mesh_parameter_node : public graph_node {
 ...
 public:
  prepare_hand_mesh_parameter_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }
  ...
  virtual void process(std::string input_name, graph_message_ptr message) override {
    if (auto obj_msg = std::dynamic_pointer_cast<object_message>(message)) {
      ...
      const auto pred_pose_rotmat =
          rot6d_to_rotmat(pred_pose.view<3>({6, pred_pose.shape[0] / 6, pred_pose.shape[1]}));
      const auto hand_pose = pred_pose_rotmat
                                 .view<4>({pred_pose_rotmat.shape[0], pred_pose_rotmat.shape[1],
                                           pred_pose_rotmat.shape[2], pred_pose_rotmat.shape[3]},
                                          {0, 0, 0, 0})
                                 .contiguous();
      const auto pred_orient_rotmat =
          rot6d_to_rotmat(pred_orient.view<3>({6, pred_orient.shape[0] / 6, pred_orient.shape[1]}));
...
class optimize_body_mesh_parameter_node : public graph_node {
 ...
 public:
  optimize_body_mesh_parameter_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }
    ...
    for (const auto &item : params.items()) {
      ...
      if (type == "float") {
        ...
        if (shape.size() == 2) {
          ...
          tensor<elem_type, num_dims> output_tensor(
              {static_cast<std::uint32_t>(shape.at(1)), static_cast<std::uint32_t>(shape.at(0))},
              data.data());
  ...
  tensor<float, 4> get_global_rotation(const tensor<float, 4> &global_orient,
    ...
    for (uint32_t i = 0; i < transforms.shape[3]; i++) {
      for (uint32_t j = 1; j < transforms.shape[2]; j++) {
        ...
        transforms.view<2>({4, 4, 0, 0}, {0, 0, j, i})
            .assign(abs_transform.view(), [](auto, const float value, auto...) { return value; });
      }
    }

    return transforms.view<4>({3, 3, transforms.shape[2], transforms.shape[3]}, {0, 0, 0, 0})
        .contiguous();
  }
      ...
      for (uint32_t m = 0; m < result.shape[1]; m++) {
        for (uint32_t n = 0; n < result.shape[0]; n++) {
          for (uint32_t k = 0; k < value1.shape[0]; k++) {
            result.set({n, m, i},
                       result.get({n, m, i}) + value1.get({k, m, i}) * value2.get({n, k, i}));
          }
        }
      }
    }
      ...
      for (uint32_t l = 0; l < betas.shape[0]; l++) {
        for (uint32_t m = 0; m < shape_disps.shape[2]; m++) {
          for (uint32_t k = 0; k < shape_disps.shape[1]; k++) {
            result.set({k, m, b},
                       result.get({k, m, b}) + betas.get({l, b}) * shape_disps.get({l, k, m}));
          }
        }
      }
    }
  ...
  tensor<float, 3> get_tpose(const tensor<float, 2> &betas) const {
    ...
    const auto joints =
        joint_template.view<3>({joint_template.shape[0], joint_template.shape[1], 1})
            .transform(
                blend_shapes(betas, joint_dirs).view(),
  ...
  static std::array<std::array<float, 3>, 3> quaternion_to_rotation_matrix(
    ...
    return {std::array<float, 3>{r00, r01, r02}, std::array<float, 3>{r10, r11, r12},
  ...
  static std::array<float, 3> quaternion_to_angle_axis(const std::array<float, 4> &quaternion) {
    ...
    if (std::fpclassify(sin_theta) != FP_ZERO) {
      ...
      const auto two_theta = 2.0f * ((cos_theta < 0.0f) ? std::atan2(-sin_theta, -cos_theta)
  ...
  static std::tuple<std::array<std::array<float, 3>, 3>, float> compute_twist_rotation(
    ...
    const auto projection =
        mul(norm_twist_axis, dot(norm_twist_axis, {quaternion[1], quaternion[2], quaternion[3]}));

    const auto twist_quaternion =
        normalize<4>({quaternion[0], projection[0], projection[1], projection[2]});
  ...
  static std::tuple<tensor<float, 3>, tensor<float, 1>> compute_twist_rotation(
    ...
    for (uint32_t i = 0; i < batch_size; i++) {
      ...
      const auto [twist_rotation_block, twist_angle_block] =
          compute_twist_rotation(rotation_matrix_block, twist_axis_block);
  ...
  static tensor<float, 3> batch_rodrigues(const tensor<float, 2> &rot_vec) {
    ...
    for (uint32_t i = 0; i < batch_size; i++) {
      const auto angle = norm<3>(
          {rot_vec.get({0, i}) + 1e-8f, rot_vec.get({1, i}) + 1e-8f, rot_vec.get({2, i}) + 1e-8f});

      const auto rx = rot_vec.get({0, i}) / angle;
      const auto ry = rot_vec.get({1, i}) / angle;
      const auto rz = rot_vec.get({2, i}) / angle;

      std::array<std::array<float, 3>, 3> k = {{{0, -rz, ry}, {rz, 0, -rx}, {-ry, rx, 0}}};
  ...
  virtual void process(std::string input_name, graph_message_ptr message) override {
    if (auto obj_msg = std::dynamic_pointer_cast<object_message>(message)) {
      ...
      auto pred_rotmat_body =
          rot6d_to_rotmat(pred_pose.view<3>({6, pred_pose.shape[0] / 6, pred_pose.shape[1]}));

      {
        const auto body_pose = pred_rotmat_body
                                   .view<4>({pred_rotmat_body.shape[0], pred_rotmat_body.shape[1],
                                             21, pred_rotmat_body.shape[3]},
                                            {0, 0, 1, 0})
                                   .contiguous();
        const auto global_orient =
            pred_rotmat_body
                .view<4>({pred_rotmat_body.shape[0], pred_rotmat_body.shape[1], 1,
                          pred_rotmat_body.shape[3]},
                         {0, 0, 0, 0})
                .contiguous();
        ...
        const auto pred_global_lelbow =
            body_global_pose.view<3>({3, 3, 1, 0}, {0, 0, 18, 0}).transpose({1, 0, 2}).contiguous();
        const auto pred_global_relbow =
            body_global_pose.view<3>({3, 3, 1, 0}, {0, 0, 19, 0}).transpose({1, 0, 2}).contiguous();

        const auto target_global_lwrist =
            flip_rotmat(rot6d_to_rotmat(pred_orient_lhand.view<3>(
                            {6, pred_orient_lhand.shape[0] / 6, pred_orient_lhand.shape[1]})))
                .view<3>({3, 3, 1, 0}, {0, 0, 0, 0})
                .contiguous();
        const auto target_global_rwrist =
            rot6d_to_rotmat(pred_orient_rhand.view<3>(
                                {6, pred_orient_rhand.shape[0] / 6, pred_orient_rhand.shape[1]}))
                .view<3>({3, 3, 1, 0}, {0, 0, 0, 0})
                .contiguous();
        ...
        const auto lelbow_twist_axis = ::normalize(
            tpose_joints.view<2>({tpose_joints.shape[0], 0, tpose_joints.shape[2]}, {0, 20, 0})
                .transform(tpose_joints.view<2>({tpose_joints.shape[0], 0, tpose_joints.shape[2]},
                                                {0, 18, 0}),
                           [](const float value1, const float value2, auto...) {
                             return value1 - value2;
                           }));

        const auto relbow_twist_axis = ::normalize(
            tpose_joints.view<2>({tpose_joints.shape[0], 0, tpose_joints.shape[2]}, {0, 21, 0})
                .transform(tpose_joints.view<2>({tpose_joints.shape[0], 0, tpose_joints.shape[2]},
                                                {0, 19, 0}),
                           [](const float value1, const float value2, auto...) {
                             return value1 - value2;
        ...
        const auto lelbow_twist_angle =
            std::get<1>(compute_twist_rotation(opt_lwrist, lelbow_twist_axis));
        const auto relbow_twist_angle =
            std::get<1>(compute_twist_rotation(opt_rwrist, relbow_twist_axis));
        const auto pi = 3.141592653589793f;
        const auto min_angle = -0.4f * pi;
        const auto max_angle = 0.4f * pi;

        const auto norm_lelbow_twist_angle =
            lelbow_twist_angle.transform([&](const float value, auto...) {
        ...
        const auto norm_relbow_twist_angle =
            relbow_twist_angle.transform([&](const float value, auto...) {
              if (value == std::clamp(value, min_angle, max_angle)) {
                return 0.0f;
              } else if (value > max_angle) {
                return value - max_angle;
        ...
        auto opt_lelbow =
            bmm(pred_rotmat_body.view<3>({3, 3, 1, 0}, {0, 0, 18, 0}).contiguous(), lelbow_twist);
        auto opt_relbow =
            bmm(pred_rotmat_body.view<3>({3, 3, 1, 0}, {0, 0, 19, 0}).contiguous(), relbow_twist);

        opt_lwrist.view().assign(
            pred_rotmat_body.view<3>({3, 3, 1, 0}, {0, 0, 20, 0}),
            [&](const float value1, const float value2, auto, auto, const size_t i) {
              return (vis_lhand[i] > hand_vis_th) ? value1 : value2;
            });
        opt_rwrist.view().assign(
            pred_rotmat_body.view<3>({3, 3, 1, 0}, {0, 0, 21, 0}),
            [&](const float value1, const float value2, auto, auto, const size_t i) {
              return (vis_rhand[i] > hand_vis_th) ? value1 : value2;
            });
        opt_lelbow.view().assign(
            pred_rotmat_body.view<3>({3, 3, 1, 0}, {0, 0, 18, 0}),
            [&](const float value1, const float value2, auto, auto, const size_t i) {
              return (vis_lhand[i] > hand_vis_th) ? value1 : value2;
            });
        opt_relbow.view().assign(
            pred_rotmat_body.view<3>({3, 3, 1, 0}, {0, 0, 19, 0}),
            [&](const float value1, const float value2, auto, auto, const size_t i) {
              return (vis_rhand[i] > hand_vis_th) ? value1 : value2;
            });

        pred_rotmat_body
            .view<3>({pred_rotmat_body.shape[0], pred_rotmat_body.shape[1], 0,
                      pred_rotmat_body.shape[3]},
                     {0, 0, 18, 0})
            .assign(opt_lelbow.view());
        pred_rotmat_body
            .view<3>({pred_rotmat_body.shape[0], pred_rotmat_body.shape[1], 0,
                      pred_rotmat_body.shape[3]},
                     {0, 0, 19, 0})
            .assign(opt_relbow.view());
        pred_rotmat_body
            .view<3>({pred_rotmat_body.shape[0], pred_rotmat_body.shape[1], 0,
                      pred_rotmat_body.shape[3]},
                     {0, 0, 20, 0})
            .assign(opt_lwrist.view());
        pred_rotmat_body
            .view<3>({pred_rotmat_body.shape[0], pred_rotmat_body.shape[1], 0,
                      pred_rotmat_body.shape[3]},
                     {0, 0, 21, 0})
            .assign(opt_rwrist.view());
      }
...
class project_joint_iwp_node : public graph_node {
 ...
 public:
  project_joint_iwp_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }
...
class extract_point_2d_node : public graph_node {
 ...
 public:
  extract_point_2d_node() : graph_node(), output(std::make_shared<graph_edge>(this)), index(0) {
    set_output(output);
  }
...
int main(int argc, char *argv[]) try {
  ...
  graph_edge_ptr pred_pose_face, pred_exp_face, pred_orient_face, pred_shape_face, pred_cam_face;
  {
    ...
    std::shared_ptr<frame_number_sync_node> sync1(new frame_number_sync_node());
    {
      ...
      sync1->set_input(initialize_mesh_parameter_obj->add_output("init_shape_face"),
                       "pred_shape.1");
  ...
  for (const auto &part : {"lhand", "rhand"}) {
    ...
    std::shared_ptr<frame_number_sync_node> sync_hmr_input(new frame_number_sync_node());
    {
      ...
      sync_hmr_input->set_input(
          initialize_mesh_parameter_obj->add_output(fmt::format("init_{}", part)), "pred_hand.1");
      sync_hmr_input->set_input(
          initialize_mesh_parameter_obj->add_output(fmt::format("init_shape_{}", part)),
          "pred_shape.1");
      sync_hmr_input->set_input(initialize_mesh_parameter_obj->add_output("init_orient"),
                                "pred_orient.1");
      sync_hmr_input->set_input(initialize_mesh_parameter_obj->add_output("init_cam"),
                                "pred_cam.1");
    }
  ...
  std::unordered_map<std::string, graph_edge_ptr> hands_joints;
  {
    std::shared_ptr<frame_number_sync_node> sync_mesh_input(new frame_number_sync_node());
    {
      ...
      sync_mesh_input->set_input(initialize_mesh_parameter_obj->add_output("init_rhand"),
                                 "pred_rhand");
      sync_mesh_input->set_input(initialize_mesh_parameter_obj->add_output("init_lhand"),
                                 "pred_lhand");
    ...
    std::shared_ptr<prepare_body_mesh_parameter_node> prepare_mesh_parameter(
        new prepare_body_mesh_parameter_node());
  ...
  graph_edge_ptr face_vertices;
  {
    ...
    std::shared_ptr<prepare_face_mesh_parameter_node> prepare_mesh_parameter(
        new prepare_face_mesh_parameter_node());
    prepare_mesh_parameter->set_input(sync_mesh_input->get_output());
    g->add_node(prepare_mesh_parameter);
  ...
  for (const auto &part : {"lhand", "rhand"}) {
    ...
    std::shared_ptr<prepare_hand_mesh_parameter_node> prepare_mesh_parameter(
        new prepare_hand_mesh_parameter_node());
    prepare_mesh_parameter->set_input(sync_mesh_input->get_output());
    g->add_node(prepare_mesh_parameter);
  ...
  graph_edge_ptr pred_pose_body1, pred_shape_body1, pred_cam_body1, pred_rotmat_body1;
  {
    std::vector<uint8_t> mesh_aligned_feature_model_data;
    {
      const auto model_path =
          "../sample/pymafx/data/pymafx/body_mesh_aligned_feature_encoder1.onnx";
      std::vector<uint8_t> data;
      load_model(model_path, data);
...
#if 1
  graph_edge_ptr pred_pose_face1, pred_exp_face1, pred_orient_face1, pred_shape_face1,
      pred_cam_face1;
  {
    std::vector<uint8_t> mesh_aligned_feature_model_data;
    {
      const auto model_path =
          "../sample/pymafx/data/pymafx/face_mesh_aligned_feature_encoder1.onnx";
      std::vector<uint8_t> data;
      load_model(model_path, data);
  ...
  for (const auto &part : {"lhand", "rhand"}) {
    ...
    {
      const auto model_path =
          "../sample/pymafx/data/pymafx/hand_mesh_aligned_feature_encoder1.onnx";
      std::vector<uint8_t> data;
      load_model(model_path, data);
  ...
  std::unordered_map<std::string, graph_edge_ptr> hands_joints1;
  {
    std::shared_ptr<frame_number_sync_node> sync_mesh_input(new frame_number_sync_node());
    {
      ...
      sync_mesh_input->set_input(initialize_mesh_parameter_obj->add_output("init_lhand"),
                                 "pred_lhand");
      sync_mesh_input->set_input(initialize_mesh_parameter_obj->add_output("init_face"),
                                 "pred_face");
  ...
  graph_edge_ptr pred_pose_body2, pred_shape_body2, pred_cam_body2, pred_rotmat_body2;
  {
    ...
    {
      const auto model_path =
          "../sample/pymafx/data/pymafx/body_mesh_aligned_feature_encoder2.onnx";
      std::vector<uint8_t> data;
      load_model(model_path, data);
...
#if 1
  graph_edge_ptr pred_pose_face2, pred_exp_face2, pred_orient_face2, pred_shape_face2,
      pred_cam_face2;
  {
    std::vector<uint8_t> mesh_aligned_feature_model_data;
    {
      const auto model_path =
          "../sample/pymafx/data/pymafx/face_mesh_aligned_feature_encoder2.onnx";
      std::vector<uint8_t> data;
      load_model(model_path, data);
  ...
  for (const auto &part : {"lhand", "rhand"}) {
    ...
    {
      const auto model_path =
          "../sample/pymafx/data/pymafx/hand_mesh_aligned_feature_encoder2.onnx";
      std::vector<uint8_t> data;
      load_model(model_path, data);
  ...
  std::unordered_map<std::string, graph_edge_ptr> hands_joints2;
  {
    ...
    std::shared_ptr<optimize_body_mesh_parameter_node> optimize_mesh_parameter(
        new optimize_body_mesh_parameter_node());
