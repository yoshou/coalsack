  ...
  virtual void process(std::string input_name, graph_message_ptr message) override {
    ...
    if (auto frame_msg = std::dynamic_pointer_cast<frame_message<tensor<float, 4>>>(message)) {
      ...
      const auto output_tensors =
          session.Run(Ort::RunOptions{nullptr}, input_node_names.data(), input_tensors.data(),
        ...
        if (value.IsTensor()) {
          ...
          if (shape.size() == 4) {
            ...
            tensor<float, num_dims> output_tensor(
                {static_cast<std::uint32_t>(shape.at(3)), static_cast<std::uint32_t>(shape.at(2)),
            ...
            auto msg = std::make_shared<frame_message<tensor<float, num_dims>>>();
            tensor<float, num_dims> output_tensor(
                {static_cast<std::uint32_t>(shape.at(4)), static_cast<std::uint32_t>(shape.at(3)),
...
class pre_project_node : public graph_node {
 ...
 public:
  pre_project_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }
...
COALSACK_REGISTER_NODE(pre_project_node, graph_node)
 ...
 public:
  ...
  std::tuple<tensor<float, 4>, std::vector<std::array<float, 3>>> get_voxel(
    ...
    const auto merged_cubes =
        cubes
            .transform(bounding, [](const float value1, const float value2,
                                    auto...) { return value1 * value2; })
            .sum<1>({0})
            .transform(bounding_count, [](const float value1, const float value2, auto...) {
...
COALSACK_REGISTER_NODE(project_node, graph_node)
 ...
 public:
  ...
  virtual void process(std::string input_name, graph_message_ptr message) override {
    if (auto frame_msg = std::dynamic_pointer_cast<frame_message<tensor<float, 5>>>(message)) {
      ...
      const auto dst =
          src.view<3>(
                 {src.shape[0] * src.shape[1] * src.shape[2], 0, 0, src.shape[3], src.shape[4]}, {})
              .transform([this](const float value, auto...) { return beta * value; })
              .softmax(0)
              .transform_expand<1>({3},
                                   [this, &grid](const float value, const size_t i, auto...) {
...
class pose_visualize_node : public graph_node {
  std::string output_dir = "runs/voxelpose_vis";
 ...
 public:
  ...
  static std::vector<cv::Point2f> project_to_input_image(
      ...
      const roi_data &roi) {
    const auto proj = project_node::project_point(points3d, camera);
    const auto image_size = cv::Size2f(960, 512);
    const auto center =
...
int main(int argc, char *argv[]) try {
  ...
  heatmap_proposal_sync->set_input(map_data2->add_output("grid_center"), "grid_center");
  ...
  heatmap_proposal_sync->set_initial_ids({"grid_center", "heatmaps", "proposal"});
  ...
  vis_sync->set_input(soft_argmax->get_output(), "poses");
  ...
  visualize->set_output_dir("runs/voxelpose_vis");
