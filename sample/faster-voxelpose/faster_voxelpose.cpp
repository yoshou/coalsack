 ...
 public:
  ...
  virtual void initialize() override {
    for (const auto &sequence : sequence_list) {
      ...
      {
        const auto camera_file =
            fs::path(data_dir) / sequence / fmt::format("calibration_{:s}.json", sequence);
        ...
        for (const auto &cam : calib["cameras"]) {
          ...
          if (std::find(camera_list.begin(), camera_list.end(), std::make_tuple(panel, node)) ==
              camera_list.end()) {
            continue;
          }
      ...
      for (size_t i = 0; i < annotation_files.size(); i++) {
        for (const auto &[camera_panel, camera_node] : camera_list) {
          ...
          const auto image_file =
              (fs::path(data_dir) / sequence / "hdImgs" / prefix / (prefix + postfix))
    ...
    th.reset(new std::thread([&]() {
      ...
      for (size_t i = 0; running.load() && i < data.size(); i += num_view, ++frame_number) {
        ...
        for (size_t j = 0; j < num_view; j++) {
          ...
          tensor<uint8_t, 4> input_img_tensor(
          ...
          const auto input_img_tensor_f = input_img_tensor.cast<float>().transform(
              [this](const float value, const size_t w, const size_t h, const size_t c,
                     const size_t n) { return value / 255.0f; });
...
class normalize_node : public graph_node {
 ...
 public:
  normalize_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }
    ...
    if (auto frame_msg = std::dynamic_pointer_cast<frame_message<tensor<float, 4>>>(message)) {
      ...
      const auto dst =
          src.transform([this](const float value, const size_t w, const size_t h, const size_t c,
                               const size_t n) { return (value - mean[c]) / std[c]; });
...
class onnx_runtime_session_pool {
  ...
  std::unordered_map<std::vector<uint8_t>, std::shared_ptr<onnx_runtime_session>, vector_hash>
      sessions;
  std::mutex mtx;
 ...
 public:
  ...
  static std::tuple<size_t, const float *, std::shared_ptr<frame_message_base>> get_input_data(
    ...
    if constexpr (num_dims > 1) {
      ...
      return std::make_tuple(static_cast<size_t>(0), nullptr,
                             std::shared_ptr<frame_message_base>());
    }
  }
...

    {
...

      {
        ...
        output_tensors = session->session.Run(Ort::RunOptions{nullptr}, input_name_strs.data(),
                                              ...
                                              output_name_strs.data(), output_name_strs.size());
      }
    ...
    if (auto frame_msg = std::dynamic_pointer_cast<frame_message<tensor<float, 4>>>(message)) {
      ...
      for (size_t input = 0; input < interpreter->inputs().size(); input++) {
        ...
        for (size_t dim = 0; dim < 4; dim++) {
          input_tensor_view.shape[dim] =
              input_tensor->dims->data[input_tensor->dims->size - 1 - dim];
        }
        input_tensor_view.stride[0] = 1;
        for (size_t dim = 1; dim < 4; dim++) {
          input_tensor_view.stride[dim] =
              input_tensor_view.stride[dim - 1] * input_tensor_view.shape[dim - 1];
        }
      ...
      for (size_t output = 0; output < interpreter->outputs().size(); output++) {
        ...
        if (output_tensor->dims->size == 4) {
          ...
          for (size_t dim = 0; dim < num_dims; dim++) {
            output_tensor_shape[dim] =
                output_tensor->dims->data[output_tensor->dims->size - 1 - dim];
          }
          std::array<uint32_t, num_dims> output_tensor_stride;
          output_tensor_stride[0] = 1;
          for (size_t dim = 1; dim < num_dims; dim++) {
            output_tensor_stride[dim] =
                output_tensor_stride[dim - 1] * output_tensor_shape[dim - 1];
          }
          ...
          for (size_t dim = 1; dim < num_dims; dim++) {
            output_tensor_stride[dim] =
                output_tensor_stride[dim - 1] *
                output_tensor->dims->data[output_tensor->dims->size - 1 - (dim - 1)];
          }
...
class pre_project_whole_node : public graph_node {
 ...
 public:
  pre_project_whole_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }

  virtual std::string get_proc_name() const override { return "pre_project_whole_node"; }
...
class project_whole_node : public graph_node {
 ...
 public:
  project_whole_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }

  virtual std::string get_proc_name() const override { return "project_whole_node"; }
    ...
    for (int32_t x = 0; x < cube_size.at(0); x++) {
      for (int32_t y = 0; y < cube_size.at(1); y++) {
        for (int32_t z = 0; z < cube_size.at(2); z++) {
          const auto gridx = -grid_size.at(0) / 2 + grid_size.at(0) * x / (cube_size.at(0) - 1) +
                             grid_center.at(0);
          const auto gridy = -grid_size.at(1) / 2 + grid_size.at(1) * y / (cube_size.at(1) - 1) +
                             grid_center.at(1);
          const auto gridz = -grid_size.at(2) / 2 + grid_size.at(2) * z / (cube_size.at(2) - 1) +
                             grid_center.at(2);
  ...
  static std::vector<std::array<float, 2>> project_point(const std::vector<std::array<float, 3>> &x,
    ...
    std::transform(x.begin(), x.end(), std::back_inserter(points), [&](const auto &p) {
      ...
      const auto cam_x = pt_x * camera.rotation[0][0] + pt_y * camera.rotation[0][1] +
                         pt_z * camera.rotation[0][2];
      const auto cam_y = pt_x * camera.rotation[1][0] + pt_y * camera.rotation[1][1] +
                         pt_z * camera.rotation[1][2];
      const auto cam_z = pt_x * camera.rotation[2][0] + pt_y * camera.rotation[2][1] +
                         pt_z * camera.rotation[2][2];
  ...
  tensor<float, 4> get_voxel(const std::vector<tensor<float, 4>> &heatmaps,
                             const std::vector<camera_data> &cameras,
    ...
    for (size_t c = 0; c < num_cameras; c++) {
      ...
      std::transform(xy.begin(), xy.end(), std::back_inserter(sample_grid), [&](const auto &p) {
        ...
        const auto x2 =
            x1 * transf.at<float>(0, 0) + y1 * transf.at<float>(0, 1) + transf.at<float>(0, 2);
        const auto y2 =
            x1 * transf.at<float>(1, 0) + y1 * transf.at<float>(1, 1) + transf.at<float>(1, 2);
      ...
      auto camera_cubes =
          cubes.view({0, cubes.shape[1], cubes.shape[2], cubes.shape[3]}, {c, 0, 0, 0});
    ...
    const auto merged_cubes =
        cubes.sum<1>({0}).view().transform([num_cameras](const float value, auto...) {
    ...
    if (auto obj_msg = std::dynamic_pointer_cast<object_message>(message)) {
      ...
      for (const auto &[name, field] : obj_msg->get_fields()) {
        if (name == "heatmaps") {
          ...
          if (const auto grid_center_msg =
                  std::dynamic_pointer_cast<frame_message<tensor<float, 1>>>(field)) {
      ...
      const auto cubes =
          get_voxel(heatmaps, cameras, rois,
                    {grid_center.get({0}), grid_center.get({1}), grid_center.get({2})});
...
class center_decode_node : public graph_node {
 ...
 public:
  center_decode_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }

  virtual std::string get_proc_name() const override { return "center_decode_node"; }
  ...
  static tensor<uint64_t, 2> get_index_2d(const tensor<uint64_t, 1> &indices,
                                          const std::array<uint64_t, 2> &shape) {
    const auto num_people = indices.shape[3];
    const auto result = indices.transform_expand<1>({2}, [shape](const uint64_t value, auto...) {
  ...
  static std::tuple<tensor<float, 1>, tensor<uint64_t, 2>, tensor<uint64_t, 1>> nms2d(
      const tensor<float, 4> &hm, const size_t max_num) {
    const auto nms = max_pool(hm);
    const auto [topk_values, topk_index] =
        nms.view<1>({hm.shape[0] * hm.shape[1] * hm.shape[2]}).topk(max_num);
  ...
  virtual void process(std::string input_name, graph_message_ptr message) override {
    if (auto obj_msg = std::dynamic_pointer_cast<object_message>(message)) {
      const auto base_frame_msg =
          std::dynamic_pointer_cast<frame_message_base>(obj_msg->get_field("hm"));

      const auto &hm =
          std::dynamic_pointer_cast<frame_message<tensor<float, 4>>>(obj_msg->get_field("hm"))
              ->get_data();
      const auto &size =
          std::dynamic_pointer_cast<frame_message<tensor<float, 4>>>(obj_msg->get_field("size"))
              ->get_data();
      const auto &feats =
          std::dynamic_pointer_cast<frame_message<tensor<float, 4>>>(obj_msg->get_field("feats"))
              ->get_data();
      ...
      const auto flatten_feats =
          feats.view<3>({feats.shape[0], feats.shape[1] * feats.shape[2], feats.shape[3]});
  ...
  virtual void process(std::string input_name, graph_message_ptr message) override {
    if (auto obj_msg = std::dynamic_pointer_cast<object_message>(message)) {
      const auto frame_msg =
          std::dynamic_pointer_cast<frame_message_base>(obj_msg->get_field("topk_2d_confs"));

      const auto &topk_2d_confs = std::dynamic_pointer_cast<frame_message<tensor<float, 1>>>(
                                      obj_msg->get_field("topk_2d_confs"))
                                      ->get_data();
      const auto &topk_2d_index = std::dynamic_pointer_cast<frame_message<tensor<uint64_t, 2>>>(
                                      obj_msg->get_field("topk_2d_index"))
                                      ->get_data();
      const auto &topk_size = std::dynamic_pointer_cast<frame_message<tensor<float, 2>>>(
                                  obj_msg->get_field("topk_size"))
                                  ->get_data();
      const auto &feat_1d =
          std::dynamic_pointer_cast<frame_message<tensor<float, 3>>>(obj_msg->get_field("feat_1d"))
              ->get_data();

      const auto [topk_1d_confs, topk_1d_index] =
          feat_1d.view<2>({feat_1d.shape[0], feat_1d.shape[1] * feat_1d.shape[2]}).topk(1);

      const auto topk_index = tensor<uint64_t, 2>::concat<0>({topk_2d_index, topk_1d_index});

      const auto topk_confs = topk_2d_confs.view().transform(
          topk_1d_confs.squeeze(0),
          [](const auto &value1, const auto &value2, auto...) { return value1 * value2; });
      ...
      proposal_centers.view<1>({0, proposal_centers.shape[1]}, {4, 0})
          .assign(topk_confs.view(), [](auto, const float value, auto...) { return value; });
      ...
      proposal_centers.view({2, proposal_centers.shape[1]}, {5, 0})
          .assign(topk_size.view(), [this](auto, const float value, auto...) { return value; });
...
class iterate_proposal_node : public graph_node {
 ...
 public:
  iterate_proposal_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }
    ...
    if (auto obj_msg = std::dynamic_pointer_cast<object_message>(message)) {
      ...
      for (uint32_t i = 0; i < proposal.shape[1]; i++) {
        if (proposal.get({3, i}) >= 0) {
          ...
          proposal_center_msg->set_data(
              std::move(proposal.view<1>({proposal.shape[0], 0}, {0, i}).contiguous()));
...
class project_individual_node : public graph_node {
 ...
 public:
  project_individual_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }

  virtual std::string get_proc_name() const override { return "project_individual_node"; }

  std::array<float, 3> get_grid_size() const { return grid_size; }
  void set_grid_size(const std::array<float, 3> &value) { grid_size = value; }
  std::array<float, 3> get_individual_grid_size() const { return individual_grid_size; }
  void set_individual_grid_size(const std::array<float, 3> &value) { individual_grid_size = value; }
  std::array<int32_t, 3> get_cube_size() const { return cube_size; }
  void set_cube_size(const std::array<int32_t, 3> &value) { cube_size = value; }
  ...
  virtual void initialize() override {
    for (size_t i = 0; i < 3; i++) {
      fine_cube_size[i] =
          static_cast<int32_t>(grid_size[i] / individual_grid_size[i] * (cube_size[i] - 1)) + 1;
    }
  }

  static std::vector<std::array<float, 3>> compute_grid(const std::array<float, 3> &grid_size,
  ...
  std::tuple<tensor<float, 4>, std::array<float, 3>> get_voxel(
      const std::vector<tensor<float, 4>> &heatmaps, const std::vector<camera_data> &cameras,
      const std::vector<roi_data> &rois, const std::array<float, 3> &center,
      const std::array<float, 2> &size) const {
    ...
    for (size_t i = 0; i < 3; i++) {
      ...
      bias[i] = -individual_grid_size[i] / 2.0f / grid_size[i] * (fine_cube_size[i] - 1) -
                scale[i] * (grid_center[i] - grid_size[i] / 2.0f);
    }
    ...
    for (size_t i = 0; i < 3; i++) {
      ...
      offset[i] = static_cast<float>(center_tl[i]) / (fine_cube_size[i] - 1) * grid_size[i] -
                  grid_size[i] / 2.0f + individual_grid_size[i] / 2.0f;
    }
    ...
    if (auto obj_msg = std::dynamic_pointer_cast<object_message>(message)) {
      ...
      for (const auto &[name, field] : obj_msg->get_fields()) {
        if (name == "heatmaps") {
          ...
          if (const auto proposal_center_msg =
                  std::dynamic_pointer_cast<frame_message<tensor<float, 1>>>(field)) {
      ...
      const auto [cubes, offset] =
          get_voxel(heatmaps, cameras, rois,
                    {proposal_center.get({0}), proposal_center.get({1}), proposal_center.get({2})},
                    {proposal_center.get({5}), proposal_center.get({6})});
      ...
      const auto center_grid = tensor<float, 2>::stack({
          grid_view.view<3>({2, 0, grid_view.shape[2], grid_view.shape[3]}, {0, 0, 0, 0})
              .reshape<2>({2, grid_view.shape[2] * grid_view.shape[3]}),
          grid_view
              .view<3>({2, grid_view.shape[1], 0, grid_view.shape[3]}, {0, 0, 0, 0}, {2, 1, 1, 1})
              .reshape<2>({2, grid_view.shape[1] * grid_view.shape[3]}),
          grid_view.view<3>({2, grid_view.shape[1], grid_view.shape[2], 0}, {1, 0, 0, 0})
              .reshape<2>({2, grid_view.shape[1] * grid_view.shape[2]}),
      });
...
class project_orthogonal_node : public graph_node {
 ...
 public:
  project_orthogonal_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }

  virtual std::string get_proc_name() const override { return "project_orthogonal_node"; }
...
class soft_argmax_node : public graph_node {
 ...
 public:
  soft_argmax_node() : graph_node(), output(std::make_shared<graph_edge>(this)) {
    set_output(output);
  }
  ...
  virtual void process(std::string input_name, graph_message_ptr message) override {
    if (auto frame_msg = std::dynamic_pointer_cast<frame_message<tensor<float, 4>>>(message)) {
      ...
      const auto softmax_src =
          src.view<3>({src.shape[0] * src.shape[1], 0, src.shape[2], src.shape[3]}, {})
              .transform([this](const float value, auto...) { return beta * value; })
              .softmax(0);

      const auto dst = softmax_src.unsqueeze(0)
                           .transform(grid.unsqueeze(2), [](const auto value1, const auto value2,
                                                            auto...) { return value1 * value2; })
                           .sum<1>({1});

      const auto num = softmax_src.shape[1] * softmax_src.shape[2];

      const auto confs = softmax_src.unsqueeze().contiguous().max<1>({0}).sum<2>({0, 1}).transform(
          [num](const float value, auto...) { return value / num; });
...
class fusion_node : public graph_node {
 ...
 public:
  fusion_node() : graph_node(), output(std::make_shared<graph_edge>(this)) { set_output(output); }

  template <typename Archive>
  void serialize(Archive &archive) {}

  virtual std::string get_proc_name() const override { return "fusion_node"; }

  static tensor<float, 2> fuse_pose_preds(const tensor<float, 3> &pose_preds,
                                          const tensor<float, 3> &weights) {
    ...
    const auto norm_x_weight = x_weight.view().transform(
        x_weight.sum<1>({0}).unsqueeze(0),
        [](const auto value1, const auto value2, auto...) { return value1 / value2; });
    const auto norm_y_weight = y_weight.view().transform(
        y_weight.sum<1>({0}).unsqueeze(0),
        [](const auto value1, const auto value2, auto...) { return value1 / value2; });
    const auto norm_z_weight = z_weight.view().transform(
        z_weight.sum<1>({0}).unsqueeze(0),
        [](const auto value1, const auto value2, auto...) { return value1 / value2; });
    ...
    const auto x_pred =
        x_weight1.transform(x_pred1, mul).transform(x_weight2.transform(x_pred2, mul), add);

    const auto y_pred =
        y_weight1.transform(y_pred1, mul).transform(y_weight2.transform(y_pred2, mul), add);

    const auto z_pred =
        z_weight1.transform(z_pred1, mul).transform(z_weight2.transform(z_pred2, mul), add);
  ...
  virtual void process(std::string input_name, graph_message_ptr message) override {
    if (auto obj_msg = std::dynamic_pointer_cast<object_message>(message)) {
      const auto frame_msg =
          std::dynamic_pointer_cast<frame_message_base>(obj_msg->get_field("pose_preds"));

      auto pose_preds = std::dynamic_pointer_cast<frame_message<tensor<float, 3>>>(
                            obj_msg->get_field("pose_preds"))
                            ->get_data();
      const auto &confs =
          std::dynamic_pointer_cast<frame_message<tensor<float, 1>>>(obj_msg->get_field("confs"))
              ->get_data();
      const auto &offset =
          std::dynamic_pointer_cast<frame_message_base>(obj_msg->get_field("cubes"))
              ->get_metadata<frame_message<std::array<float, 3>>>("offset")
              ->get_data();
      const auto &weights =
          std::dynamic_pointer_cast<frame_message<tensor<float, 3>>>(obj_msg->get_field("weights"))
              ->get_data();

      const std::array<float, 2> offset_xy = {offset[0], offset[1]};
      pose_preds.view<2>({2, pose_preds.shape[1], 0}, {0, 0, 0})
          .assign([&offset_xy](const auto value, const auto i, auto...) {
            return value + offset_xy[i];
          });

      const std::array<float, 2> offset_xz = {offset[0], offset[2]};
      pose_preds.view<2>({2, pose_preds.shape[1], 0}, {0, 0, 1})
          .assign([&offset_xz](const auto value, const auto i, auto...) {
            return value + offset_xz[i];
          });

      const std::array<float, 2> offset_yz = {offset[1], offset[2]};
      pose_preds.view<2>({2, pose_preds.shape[1], 0}, {0, 0, 2})
          .assign([&offset_yz](const auto value, const auto i, auto...) {
