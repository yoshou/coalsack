...
try
{
    ...
    std::shared_ptr<coalsack::depthai_color_camera_node> capture(new coalsack::depthai_color_camera_node());
    g->add_node(capture);
    ...
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(static_cast<std::int64_t>(1000.0 / 30.0)));
        if (viz)
        {
