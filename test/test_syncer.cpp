...
TEST(SyncerTest, Test1) {
  ...
  syncer.start(
      std::make_shared<typename syncer_type::callback_type>([&](const std::map<int, int>& frames) {
...
TEST(SyncerTest, Test2) {
  using syncer_type = stream_syncer<int, int, frame_number>;
  syncer_type syncer;

  int num_stream = 10;

  // First data outputs immediately because there are no other streams you need to wait.
  syncer.sync(0, 0, frame_number(0));

  const auto frame1_time = 1;
  const auto frame2_time = 2;

  // The other stream are blocked to wait stream 0.
  for (int i = 1; i < num_stream; i++) {
    syncer.sync(i, 0, frame_number(frame1_time));
  }

  // Old data is output
  syncer.sync(0, 0, frame_number(frame2_time));
  ...
  // All stream data ready
  for (int i = 1; i < num_stream; i++) {
    syncer.sync(i, i, frame_number(frame2_time));
  }
}