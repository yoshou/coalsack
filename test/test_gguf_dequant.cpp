...
TEST(GgufDequantTest, Fp16RoundTrip) {
  std::vector<float> test_values = {0.0f,  1.0f,     -1.0f,     2.0f,   0.5f,
                                    0.25f, 3.14159f, -3.14159f, 100.0f, -100.0f};

  for (float val : test_values) {
    uint16_t fp16 = fp32_to_fp16(val);
    float converted = fp16_to_fp32(fp16);
    // Allow for precision loss in fp16
    EXPECT_NEAR(converted, val, std::abs(val) * 0.001f + 0.0001f) << "Failed for value: " << val;
  }
}
...
TEST(GgufDequantTest, DequantizeTensorF32) {
  ...
  bool result =
      dequantize_tensor(reinterpret_cast<uint8_t*>(src.data()), dst.data(), n, ggml_type::F32);
...
TEST(GgufDequantTest, DequantizeTensorF16) {
  ...
  bool result =
      dequantize_tensor(reinterpret_cast<uint8_t*>(src.data()), dst.data(), n, ggml_type::F16);
...
TEST(GgufDequantTest, DequantizeQ5_0Basic) {
  ...
  *scale_ptr = fp32_to_fp16(1.0f);
  ...
  for (int i = 0; i < QK5_0 / 2; ++i) {
    src[2 + 4 + i] =
        0x00;  // 0x00 means both nibbles are 0, which maps to -16 before adding high bit
  }
...
TEST(GgufDequantTest, DequantizeQ5_K_Complex) {
  ...
  EXPECT_GT(high_bit_variations, 50)
      << "Not enough variations, high bits may not be decoded correctly";
}
